...
from .models import Measurement, MeasurementAgg, Legend
...
def build_pollution_scale():
    legends = Legend.objects.prefetch_related("levels")
    pollution_scale = {}
    for legend in legends:
        levels = legend.levels.all()
        pollution_scale[legend.pollutant] = {
            "unit": legend.unit,
            "limits": [level.limit for level in levels],
            "labels": [level.label for level in levels],
            "colors": [level.color for level in levels],
        }
    return pollution_scale


def main(request):
    geojson = load_coord(str(PROJECT_PATH / "static" / "geo" / "wojewodztwa-max.geojson"))
    coord_provinces = load_coord(str(PROJECT_PATH / "static" / "geo" / "coord_provinces.json"))
    pollution_scale = build_pollution_scale()
    ...
    return render(request, 'map/map.html',
                  context={"geojson": geojson,
                           ...
                           'coord_provinces': coord_provinces, "pollution_scale": pollution_scale,
                           "years": years})
